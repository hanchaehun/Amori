...
```bash
# 1) Neon 프로젝트 생성 → 연결 문자열 복사
# 2) postgresql:// → postgresql+asyncpg:// 로 접두사 변경 후 .env의 DATABASE_URL에
# 3) 스키마 생성 (pgvector 확장 포함)
alembic upgrade head
# 4) URL을 팀 시크릿 채널에 공유 (절대 커밋 금지 — .env는 gitignore)
```
