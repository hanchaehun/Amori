...
class HuggingFaceLLMProvider(LLMProvider):
    """Calls the LLM module deployed on a HuggingFace Inference Endpoint.

    Parameters
    ----------
    base_url:
        Root URL of the inference endpoint (e.g. ``https://xyz.endpoints.huggingface.cloud``).
    api_token:
        HuggingFace API bearer token.
    """
    ...
    async def _post_json(self, path: str, payload: dict) -> dict:
        ...
        async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
            try:
                ...
                logger.error("Cannot connect to LLM endpoint: %s", exc)
                raise RuntimeError(
                    f"Cannot connect to LLM endpoint at {self._base_url}"
                ) from exc
            except httpx.HTTPStatusError as exc:
                logger.error(
                    "LLM endpoint returned %s for %s: %s",
                    exc.response.status_code,
                    path,
                    exc.response.text[:500],
                )
                raise RuntimeError(
                    f"LLM endpoint error {exc.response.status_code} on {path}"
                ) from exc
